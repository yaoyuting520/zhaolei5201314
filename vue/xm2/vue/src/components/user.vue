<template>
  <div class="content">
    <div class="header">
      ...
      <ul class="clear">
        <li>
          <span>{{this.$route.query.userData.follow}}</span>
          <p>关注</p>
        </li>
        <li>
          <span>{{this.$route.query.userData.fans}}</span>
          <p class="end">粉丝</p>
        </li>
      </ul>
    </div>
...
export default {
  beforeRouteEnter(to, from, next) {
    axios({
      ...
      if (res.data.hasSession) {
        to.query.userData = res.data; //路由传递 预载数据
        next();
      } else {
        next("/login");
      }
  ...
  methods: {
    logout() {//注销
      axios({
        url: "http://localhost:3000/delsess"//给服务器发个请求,
      }).then(res => {
        if(res.error==0){//需要服务器返回对象
